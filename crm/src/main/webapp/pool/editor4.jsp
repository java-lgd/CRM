		...
		<div class="layui-form-item">
			...
			<div class="layui-form-item">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="hidden" name="operatorids"> <input
							type="text" name="operatornames" readonly="readonly"
							autocomplete="off" placeholder="请输入" class="layui-input"
							id="demo">
					</div>
				</div>
					...
					<script type="text/javascript">
						...
						tableSelect
								.render({
									...
									table : { //定义表格参数，与LAYUI的TABLE模块一致，只是无需再定义表格elem
										...
										var NEWJSON = []
										var NEWJSON1 = []
										layui.each(data.data, function(index,
												item) {
											NEWJSON.push(item.name)
											NEWJSON1.push(item.id)
										})
										elem.val(NEWJSON.join(","))
										$("[name=operatorids]").val(
												NEWJSON1.join(","));
									}
								});
						...
						function init() {
							$.post("../Client/edit", {
								...
								render('myform', json);
								getarray("../Client/getSexname", "[name=sex]",
										json.sex);
								getarray("../Client/getStatusname",
										"[name=status]", json.status);
								getarray("../Client/getLinkname",
										"[name=linkstatus]", json.linkstatus);
								getarray("../Client/getClientsname",
										"[name=clientstatus]",
										json.clientstatus);
								getarray("../Client/getPurposename",
										"[name=purposestatus]",
										json.purposestatus);
								getarray("../Client/getAssessname",
										"[name=assessstatus]",
										json.assessstatus);
								getarray("../Client/getExecname",
										"[name=execstatus]", json.execstatus);
								getlist("../Client/getTypes",
										"[name=clienttypeid]",
										json.clienttypeid);
								getlist("../Client/getOperator",
										"[name=createoperatorid]",
										json.createoperatorid);
								getlist("../Client/getSrc", "[name=srcid]",
										json.srcid);
							}, "json");
